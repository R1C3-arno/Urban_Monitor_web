...
import { MAPTILER_KEY, DATA_API_URL } from '../constants/foodConfig';
import { createMarkerElement } from '../utils/markerUtils';
import { createFoodPopupHTML } from '../utils/foodPopupTemplate.js';

const useFoodMap = (mapContainer) => {
    ...
    const showPopup = (props, coordinates) => {
        new maptilersdk.Popup({ offset: 25 })
            ...
            .setHTML(createFoodPopupHTML(props))
            .addTo(map.current);
    };
    ...
    useEffect(() => {
        ...
        map.current.on('load', async () => {
            try {
                ...
                console.log('Loaded Food Store Data:', backendData);
                ...
                console.error('Error loading food store data:', error);
            }
        });
    }, []);
