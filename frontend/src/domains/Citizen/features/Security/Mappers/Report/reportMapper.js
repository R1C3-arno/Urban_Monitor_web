import { SecurityReport } from "../../Models/Report/SecurityReport.js";
import {
    SECURITY_REPORT_CONFIG,
    SECURITY_REPORT_ERROR_MESSAGES
} from "../../Config/reportConfig.js";

/**
 * UI Form → Domain Model
 */
export const mapFormToSecurityReport = (formData) => {
    if (!formData.title) throw new Error(SECURITY_REPORT_ERROR_MESSAGES.TITLE_REQUIRED);
    if (formData.title.length < SECURITY_REPORT_CONFIG.TITLE_MIN_LENGTH)
        throw new Error(SECURITY_REPORT_ERROR_MESSAGES.TITLE_TOO_SHORT);

    if (
        !formData.description ||
        formData.description.length < SECURITY_REPORT_CONFIG.DESCRIPTION_MIN_LENGTH
    ) {
        throw new Error(SECURITY_REPORT_ERROR_MESSAGES.DESCRIPTION_TOO_SHORT);
    }

    if (!formData.type) {
        throw new Error(SECURITY_REPORT_ERROR_MESSAGES.TYPE_REQUIRED);
    }

    if (formData.lat === undefined || formData.lng === undefined)
        throw new Error(SECURITY_REPORT_ERROR_MESSAGES.LOCATION_REQUIRED);

    return new SecurityReport({
        title: formData.title,
        description: formData.description,
        type: formData.type,
        severity: formData.severity || "MEDIUM",
        lat: formData.lat,
        lng: formData.lng,
        image: formData.image || null,
    });
};

/**
 * Domain Model → API DTO
 */
export const mapSecurityReportToApi = (report) => {
    return report.toJSON();
};

/**
 * API Response → UI
 */
export const mapApiToSecurityReportResponse = (apiData) => {
    return {
        id: apiData.id,
        status: apiData.status,
        createdAt: apiData.createdAt,
    };
};